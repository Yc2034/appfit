...
- Scope:
  ...
  - 将训练时长从“周级汇总”改为“月级分类汇总”，并以堆叠柱状图展示各类别累计时长。
  - 训练图需展示分类 legend，便于区分跑步/游泳/HIIT/力量/腹部等类别贡献。
...
- Acceptance Criteria:
  ...
  - 训练时长图改为“按月 + 按类别堆叠”的柱状图，不再展示周标签柱状图。
  - 至少支持跑步、游泳、HIIT、力量训练、腹部训练 5 类并显示 legend。
  - 同一月份各类别分钟数可叠加为该月总训练时长。
  - 曲线图视觉风格与现有 Design System 保持一致，且在深浅色模式下可读性良好。
  - 不影响 Tab 1/Tab 2 和体重曲线渲染。
- UI Reference: 低饱和、清爽、偏实用导向；强化“月度趋势”的可读性与线条质感。
- Notes: 已交付。体重图为月度只读；训练时长图已切换为按月分类堆叠柱状图（含 legend），并在 `progress_seed.json` 提供多类别月度样例数据。
